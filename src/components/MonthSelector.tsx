        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5 max-w-2xl">
            ...
            <h3 className="text-2xl sm:text-3xl font-serif font-bold text-white tracking-tight">
              {currentMonthData.name} in India
            </h3>
          ...
          <div className="flex flex-wrap md:flex-col items-start md:items-end gap-1.5 shrink-0">
            <span className="text-[10px] uppercase font-bold tracking-widest text-[#E6BE8A]">Top Highlights:</span>
            <div className="flex flex-wrap gap-1.5 justify-start md:justify-end">
              {currentMonthData.topFestivals.slice(0, 3).map((item, idx) => (
                <span
                  key={idx}
                  className="text-xs bg-white/10 backdrop-blur-sm text-white px-3 py-1 rounded-full border border-white/20 font-medium"
                >
                  {item}
                </span>
              ))}
